...
const AnchorPointOverlay = () => {
  ...
  // FIX: When you change the pivot, ALL old keyframes need their x/y updated 
  // so the object doesn't jump wildly during playback.
  const updateKeyframesForAnchorChange = useCallback((objId, oldAx, oldAy, newAx, newAy, width, height) => {
    setKeyframes(prev => {
      ...
      const updatedObjKfs = prev[objId].map(kf => {
        ...
        return {
          ...kf, properties: { ...kf.properties, x: x + dx_global, y: y + dy_global }
        };
      });
  ...
  const handleMouseDown = (e) => {
    ...
    setIsDragging(true);
    setDragStartAnchor(getAnchorValues());
  };
  ...
  const handleMouseMove = useCallback((e) => {
    ...
    setCanvasObjects(prev => prev.map(obj => 
      obj.id === selectedObject ? { ...obj, anchorX: newAnchorX, anchorY: newAnchorY } : obj
    ));
  ...
  const handleMouseUp = useCallback(() => {
    ...
    if (dragStartAnchor && selectedObject && fabricCanvas) {
      ...
      if (Math.abs(currentAnchor.anchorX - dragStartAnchor.anchorX) > 0.001 || 
          Math.abs(currentAnchor.anchorY - dragStartAnchor.anchorY) > 0.001) {
        const fabricObject = findFabricObjectById(fabricCanvas, selectedObject);
        if (fabricObject) {
          updateKeyframesForAnchorChange(selectedObject, dragStartAnchor.anchorX, dragStartAnchor.anchorY, 
            currentAnchor.anchorX, currentAnchor.anchorY, fabricObject.width, fabricObject.height);
        }
      }
    }
  ...
  const handleDoubleClick = (e) => {
    ...
    setCanvasObjects(prev => prev.map(obj => 
      obj.id === selectedObject ? { ...obj, anchorX: 0.5, anchorY: 0.5 } : obj
    ));

    updateKeyframesForAnchorChange(selectedObject, oldAnchor.anchorX, oldAnchor.anchorY, 
      0.5, 0.5, fabricObject.width, fabricObject.height);
  };
  ...
  if (!anchorEditMode) {
    ...
    return (
      <>
        <Box sx={{ position: 'absolute', left: anchorPosition.x, top: anchorPosition.y, width: 18, height: 18, transform: 'translate(-50%, -50%)', pointerEvents: 'none', zIndex: 999 }}>
        ...
        <Box sx={{ position: 'absolute', left: anchorPosition.x + 12, top: anchorPosition.y - 20, bgcolor: ANCHOR_COLOR_BG, color: '#000', px: 0.5, py: 0.15, borderRadius: 0.5, fontSize: '9px', fontWeight: 700, pointerEvents: 'none', zIndex: 999, whiteSpace: 'nowrap', border: '1px solid rgba(0,0,0,0.3)' }}>
  ...
  return (
    <>
      <Box sx={{ position: 'absolute', left: anchorPosition.boundLeft - 2, top: anchorPosition.boundTop - 2, width: anchorPosition.boundWidth + 4, height: anchorPosition.boundHeight + 4, border: `2px dashed ${ANCHOR_COLOR}`, borderRadius: '2px', pointerEvents: 'none', zIndex: 999, boxShadow: `0 0 6px ${ANCHOR_COLOR}40` }} />
      {[[0, 0], [1, 0], [0, 1], [1, 1]].map(([cx, cy], i) => (
        <Box key={i} sx={{ position: 'absolute', left: anchorPosition.boundLeft + cx * anchorPosition.boundWidth - 4, top: anchorPosition.boundTop + cy * anchorPosition.boundHeight - 4, width: 8, height: 8, bgcolor: ANCHOR_COLOR, border: '1.5px solid #000', borderRadius: '1px', pointerEvents: 'none', zIndex: 999 }} />
      ))}
      <Box sx={{ position: 'absolute', left: anchorPosition.x, top: anchorPosition.y, width: 26, height: 26, transform: 'translate(-50%, -50%)', cursor: isDragging ? 'grabbing' : 'grab', zIndex: 1000, pointerEvents: 'all' }} onMouseDown={handleMouseDown} onDoubleClick={handleDoubleClick}>
      ...
      <Box sx={{ position: 'absolute', left: anchorPosition.x + 18, top: anchorPosition.y - 30, bgcolor: ANCHOR_COLOR_BG, color: '#000', px: 1, py: 0.25, borderRadius: 0.5, fontSize: '11px', fontWeight: 700, pointerEvents: 'none', zIndex: 1001, whiteSpace: 'nowrap', border: '1px solid rgba(0,0,0,0.3)', letterSpacing: '0.3px' }}>
