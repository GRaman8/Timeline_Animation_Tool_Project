/**
 * Flood Fill Utility — MS Paint-style paint bucket
 * 
 * Uses a scanline flood fill algorithm for performance on 1400×800 canvases.
 * Process:
 *   1. Render fabric canvas to an offscreen bitmap (no selection handles)
 *   2. Flood fill from click point to find the enclosed region
 *   3. Create a trimmed PNG of just the filled pixels
 *   4. Return as data URL + bounding box for fabric.Image placement
 */
...
/**
 * Scanline flood fill — much faster than naive BFS for large regions.
 */
const scanlineFill = (imageData, startX, startY, fillRgb, tolerance) => {
  ...
  // Don't fill if clicking on a pixel that already matches the fill color
  if (
  ...
  while (stack.length > 0) {
    ...
    while (y >= 0 && matches(x, y) && !filled[y * width + x]) {
      y--;
    }
    y++;
    ...
    while (y < height && matches(x, y) && !filled[y * width + x]) {
      ...
      if (!spanLeft && leftMatch) {
        stack.push([x - 1, y]);
        spanLeft = true;
      } else if (spanLeft && !leftMatch) {
        spanLeft = false;
      }

      const rightMatch = x < width - 1 && matches(x + 1, y) && !filled[y * width + (x + 1)];
      if (!spanRight && rightMatch) {
        stack.push([x + 1, y]);
        spanRight = true;
      } else if (spanRight && !rightMatch) {
        spanRight = false;
      }
...
/**
 * Create a trimmed PNG from the filled pixels.
 */
  ...
  for (let y = minY; y <= maxY; y++) {
    for (let x = minX; x <= maxX; x++) {
      if (filled[y * fullWidth + x]) {
        ...
        imgData.data[outIdx] = fillRgb.r;
        imgData.data[outIdx + 1] = fillRgb.g;
        imgData.data[outIdx + 2] = fillRgb.b;
...
/**
 * Main entry: MS Paint-style flood fill on a Fabric.js canvas.
 * 
 * @param {fabric.Canvas} fabricCanvas
 * @param {number} canvasWidth
 * @param {number} canvasHeight  
 * @param {number} clickX - click X in scene coordinates
 * @param {number} clickY - click Y in scene coordinates
 * @param {string} fillColorHex - e.g. '#ff0000'
 * @param {number} tolerance - color distance threshold (0-765), default 40
 * @returns {object|null} { dataURL, left, top, width, height, fillColor }
 */
export const performFloodFill = (fabricCanvas, canvasWidth, canvasHeight, clickX, clickY, fillColorHex, tolerance = 40) => {
  const fillRgb = hexToRgb(fillColorHex);

  // Render canvas without selection handles
  ...
  // Restore selection
  if (activeObj) {
    try { fabricCanvas.setActiveObject(activeObj); } catch(e) {}
    fabricCanvas.renderAll();
  }
